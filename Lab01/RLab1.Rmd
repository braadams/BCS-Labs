...
IPS_sampledata[varList] <- lapply(IPS_sampledata[varList], as.numeric)
```
...
class(IPS_sampledata$time) <- c("POSIXt", "POSIXct")
```
...
Num_mac_obs <- tally_mac(IPS_sampledata$mac)
```
