...
```{r}
signal_summary <- function (posX, posY, orientation, mac) {
  signal_stat = IPS_sampledata[IPS_sampledata$posX==posX &
                               IPS_sampledata$posY==posY &
                               IPS_sampledata$orientation==orientation &
                               IPS_sampledata$mac==mac,]
  signal_stat$avgSignal <- mean(signal_stat$signal)
  signal_stat$sdSignal <- sd(signal_stat$signal)
  signal_stat
}

head(signal_summary(1, 9, "E→",	"00:14:bf:b1:97:8a"))
```
...
```{r}
IPS_sampledata$ <- paste(IPS_sampledata$posX, IPS_sampledata$posY, IPS_sampledata$orientation,  IPS_sampledata$mac, sep="||")
