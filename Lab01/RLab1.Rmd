...
# Take the top 7 APs with counts over 10k obs
...
```{r}
signal_summary <- function (dataset) {
  signal_stat = dataset[, c('posX', 'posY', 'orientation', 'mac')]
  signal_stat$avgSignal <- mean(dataset$signal)
  signal_stat$sdSignal <- sd(dataset$signal)
  signal_stat
}

head(signal_summary(IPS_sampledata))
```
...
Appended_Signal_Stat <- with(IPS_sampledata, by(IPS_sampledata, Combo, function(do_nothing) do_nothing))
Appended_Signal_Stat <- Appended_Signal_Stat[!sapply(Appended_Signal_Stat, is.null)]
Appended_Signal_Stat <- lapply(Appended_Signal_Stat, signal_summary)
IPS_sampledata_Stat <- do.call("rbind", Appended_Signal_Stat)

head(IPS_sampledata_Stat)
```
