...
```{r}
IPS_sampledata <- IPS_sampledata %>% na.omit() %>% 
...
left_join(CBdata.1_10_clean_dates,weather_dat,by = c("Date" = "DATE")) -> full_data # left join by date (keep only the dates in CBdata.1_10)
...
```{r}
library(dplyr)
IPS_sampledata %>% group_by(mac) %>% tally() #Tally number of obs in mac column
```
...
IPS_sampledata <- filter(IPS_sampledata, 
                   mac == c("00:0f:a3:39:dd:cd", 
                            ...
                            "00:14:bf:b1:97:8a", 
                            "00:14:bf:b1:97:8d", 
                            "00:14:bf:b1:97:90")) #Take the top 7 APs with over 10k obs counts from Lab-1
IPS_sampledata
```
