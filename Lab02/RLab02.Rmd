...
```{r}
c("1973-04-01","1974-03-31","1975-04-06","1976-04-04","1977-04-03","1977-04-03","1978-04-02","1979-04-01","1980-03-30","1981-04-05","1982-04-04","1983-03-27")%>% 
  as.Date() -> ros_dates
```
...
```{r}
IPS_sampledata %>% group_by(mac) %>% tally() #tally number of observations in `mac` column 
```
...
IPS_sampledata <- filter(IPS_sampledata, 
                   mac == c("00:0f:a3:39:dd:cd", 
                            ...
                            "00:14:bf:b1:97:8a", 
                            "00:14:bf:b1:97:8d", 
                            "00:14:bf:b1:97:90")) #Take the top 7 AP over 10k obs from Lab-1 
IPS_sampledata
```
...
IPS_sampledata <- IPS_sampledata %>% 
  mutate(rec_orient = case_when(orientation >= 0.0 & orientation < 1.0 ~ 0,
                                orientation >= 45.0 & orientation < 46.0 ~ 45,
                                orientation >= 90.0 & orientation < 91.0 ~ 90,
                                orientation >= 135.0 & orientation  < 136.0 ~ 135,
                                orientation >= 180.0 & orientation  < 181.0 ~ 180,
                                orientation >= 225.0 & orientation < 226.0 ~ 225,
                                orientation >= 270.0 & orientation < 271.0 ~ 270,
                                orientation >= 315.0 & orientation < 316.0 ~ 315))
```
...
```{r}
c("1973-04-01","1974-03-31","1975-04-06","1976-04-04","1977-04-03","1978-04-02","1979-04-01","1980-03-30","1981-04-05","1982-04-04","1983-03-27")%>% 
  as.Date() -> ros_dates
```
...
```{r}
data.frame(Year = 1973:1983,Date = ros_dates) -> dfdates

full_join(CBdata.1_10_clean,dfdates) -> CBdata.1_10_clean_dates
```
...
```{r}
weather_dat <- select(weather,DATE,PRCP,TMIN)

left_join(CBdata.1_10_clean_dates,weather_dat,by = c("Date" = "DATE")) -> full_data
...
```
>>>>>>> 2dbaacbcf94fae6f20f0347a66596fa260c25a81
