...
## 1
```{r}
names(table(humanNames$Name)[table(humanNames$Name)>5]) -> correctNames
correct_words <- correctNames[stringdist::amatch(humanNames$Name, correctNames, maxDist = Inf)]
humanNames <- humanNames %>% transmute(Name=correct_words) 
table(humanNames)
```
...
## 3
```{r}
table(humanNames$Name)[table(humanNames$Name) > 5] %>% names() -> good_names

stringdist::stringdistmatrix(humanNames$Name, good_names) %>%
  apply(1, which.min) -> name_index

humanNames %>%
  mutate(Name = good_names[name_index]) -> humanNames_clean2
```
