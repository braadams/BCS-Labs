...
```{r cool_dat_vis}
IPS_summary %>%
  ggplot(aes(x = dist, y = median.signal)) +
  geom_point() +
  facet_wrap(~mac) + 
  labs(
...
This chart shows a general negative relationship between the distance between the reciever and access point and the recorded signal strength.

``` {r cool_data_vis}
IPS_summary %>%
  mutate(dx = x - posX, dy = y - posY) %>%
  mutate(direction = case_when(
    abs(dx) > abs(dy) & dx >= 0 ~ "Positive X direction",
    abs(dx) > abs(dy) & dx < 0  ~ "Negative X direction",
    abs(dx) < abs(dy) & dy >= 0 ~ "Positive Y direction",
    abs(dx) < abs(dy) & dy < 0  ~ "Negative Y direction"
  )) %>%
  mutate(slope = abs(dx) / abs(dy)) %>%
  filter(slope <= 0.1 | slope >= 10) %>%
  #filter(orientation %% 90 == 0) %>%
  ggplot(aes(x = orientation, y = median.signal)) +
  geom_line(aes(group = paste0(posX, posY), color = mac)) + 
...
IPS_sampledata %>%
  mutate(posXY=paste(posX, posY, sep = ",")) %>%                  # Create a new variable to uniquely identify each location.
    ...
    # geom_point(colour ='#6d91ed', show.legend = F)+             
