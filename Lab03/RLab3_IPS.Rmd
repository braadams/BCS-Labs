...
```{r loc_scatter}
# IPS_sampledata %>% ggplot(aes(posX, posY, fill=scanMac)+
#          geom_point(aes(x,y,fill=mac_locs)))
```
...
```{r summary_by_combo}
IPS_sampledata %>%  
  group_by(posX,posY,posZ,orientation,mac,x,y) %>% 
  summarize(
    n=n(), 
    mean.signal = mean(signal),
    median.signal = median(signal),
    sd.signal = sd(signal)
  ) %>%
  mutate(dist = sqrt((posX-x)^2+(posY-y)^2)) -> IPS_summary
```
